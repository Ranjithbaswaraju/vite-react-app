...
const TableUncontrolledComponent=()=>{
    ...
    const onSubmit=(e)=>{
        ...
        let formCollect=validations(usernameEntered,passwordEntered)

        if(Object.keys(formCollect).length>0){
            setFromErr(formCollect)
        }
        else{
            loginApi(usernameEntered,passwordEntered)
        }
    ...
    const loginApi=async(username,password)=>{
        try{
            const finalData=await axios.post('https://dummyjson.com/auth/login',{
                username,
                password
            })
            ...
            setApiErr("")
            // setData((prevData)=>[...prevData,{username,password}])
            // STOP adding direct [...prevData, {username, password}]

setData(prevData => {
  for (let i = 0; i < prevData.length; i++) {
    if (
      prevData[i].username === username &&
      prevData[i].password === password
    ) {
        alert("Already exist")
      return prevData;
  ...
  return [...prevData, { username, password }]; // new → ADD
});
  ...
  <span style={{color:'red'}}>{apiErr}</span>
</form>
{
    data.length>0 && (
        <div>
